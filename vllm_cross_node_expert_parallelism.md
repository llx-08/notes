...
> 系统学习入口（EP / Kernel / DeepEP / MegaMoE / EPLB）：[ep_learning/README.md](ep_learning/README.md)
>
...
更细说明见 [ep_learning/01_ep_fundamentals.md](ep_learning/01_ep_fundamentals.md) §2.3。
...
>
> **显存语义（简答）**：这是 GPU 上 **额外预分配** 的 RDMA 注册工作区（GDR 可直写），**不是**模型 hidden 原址；会挤占 KV/激活可用显存。容量按 **worst-case per-rank token 上限** 在创建时定死，运行中超上限通常 assert 而非再扩池。详见 [ep_learning/03_deepep.md](ep_learning/03_deepep.md) §3.1。
