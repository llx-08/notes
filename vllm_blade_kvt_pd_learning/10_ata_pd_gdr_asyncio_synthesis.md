...
| 《PD 分离中的 kvcache 传输优化》 | RDMA 小包、Unsignaled WR、批量 post、QP 并行 | 补充 [04 KV 发送路径](04_kv_send_path.md) 的性能动机 |
| 《PD 分离中的 GDR》 | P→D GPU Direct RDMA、内存序、Decode 干扰实验 | 补充 [05 KV 接收与完成](05_kv_receive_and_completion.md) 的硬件语义 |
| 《一定要保存 asyncio.Task 对象啊!》 | Task 弱引用、GC、后台任务可靠性 | 解释 [07 Python 协程](07_hybrid_asyncio_and_no_hang.md) 中强引用集合的由来 |
