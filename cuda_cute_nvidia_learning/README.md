...
| 顺序 | 文档 | 学习目标 |
|---:|---|---|
| 0 | [零基础导读](00_beginner_primer.md) | 建立 CPU、GPU、kernel、core、memory、architecture 的词汇表 |
| 1 | [CUDA 执行模型](01_cuda_execution_model.md) | 理解 host/device、grid/block/warp/thread、SM 调度与异步 launch |
| 2 | [SM、CUDA Core 与 Tensor Core](02_sm_cuda_core_tensor_core.md) | 看懂一个 SM 内有哪些执行与调度部件，避免把 CUDA Core 当 CPU Core |
| 3 | [GPU 内存层次](03_memory_hierarchy.md) | 理解 HBM、L2、L1/shared、register、local memory、coalescing 与 bank conflict |
| 4 | [GEMM、MMA 与 Tensor Core](04_gemm_mma_tensor_core.md) | 从矩阵乘公式走到 tile、warp MMA、数值精度和流水线 |
| 5 | [Ampere、Hopper、Blackwell 架构演进](05_ampere_hopper_blackwell.md) | 系统比较 cp.async、TMA/WGMMA、TMEM/tcgen05 和 NVLink 演进 |
| 6 | [Hopper/H20 实机导读](06_hopper_h20_lab.md) | 结合 `ecs` 的 8×H20 理解 compute capability 9.0、NVSwitch 与调试方法 |
| 7 | [Blackwell/GB200 实机导读](07_blackwell_gb200_lab.md) | 结合 `target_p_j` 的 4×GB200、SM100、CUDA 13.2 和 NV18 拓扑 |
| 8 | [CuTe DSL：Layout 与 Tensor](08_cute_dsl_layout_tensor.md) | 理解 CuTe 最核心的坐标到地址映射及 Python DSL 编译路径 |
| 9 | [CuTe DSL：Copy、MMA 与 GEMM 流水](09_cute_dsl_gemm_pipeline.md) | 将 Layout、TiledCopy、TiledMMA 与 Hopper/Blackwell 硬件操作连接起来 |
| 10 | [性能分析、实验与代码阅读路线](10_profiling_and_practice.md) | 使用 nvidia-smi、Nsight、PTX/SASS、roofline 和微基准定位瓶颈 |
| 11 | [GEMM 软件流水线深入](11_gemm_pipeline_deep_dive.md) | 定量理解延迟隐藏：四代流水线演进、stage 数推导、roofline 里 `max` 的来源，含 H20/GB200 实测 |
| 12 | [PDL：把 kernel 之间的缝隙压掉](12_pdl_programmatic_dependent_launch.md) | Programmatic Dependent Launch 的 wait/trigger 语义、warp specialization、SM 共驻算术、gemm_ar tail-hiding 与 phase counter，含 H20/GB200 实测微基准 |
| 13 | [CUDA context 与 stream：执行模型的进程视角](13_context_stream_concurrency.md) | context/UVA 层级、stream 到硬件工作队列的映射、并发的三个前提（剩余资源/NULL stream/队列数）、优先级实测无效、跨进程隔离与 CUDA IPC |
