...
#### 1.1.3 SGE：Scatter/Gather Element
...
### 1.6 Receive WQE 保存的是数据，还是 buffer 描述符
...
### 1.7 `ibv_post_send()` 到 RNIC 执行的完整流程
...
### 1.10 WQE 的状态与“outstanding”究竟指什么
...
同一个变量。更完整的三层窗口已在
[02c RoCE 拥塞、BDP 与调优](02c_roce_congestion_and_tuning.md) 第 8.5 节说明。
...
### 1.12 Signaled/Unsignaled 改变 CQE 数，不等于不占 WQE
...
### 1.13 `ibv_post_send()` 失败时，哪些 WQE 已经进入 SQ
...
### 1.14 结合 Barex：一批 Write 如何变成 WQE
...
## 3. 五种 RDMA operation 逐项拆解

### 3.1 SEND：把消息交给接收端准备好的 buffer
...
### 3.2 RDMA WRITE：发起端直接把数据推到远端指定地址
...
### 3.3 RDMA WRITE WITH IMMEDIATE：直接落位，再向远端 CQ 投递 32-bit 通知
...
### 3.5 ATOMIC：在远端做不可分割的 read-modify-write
