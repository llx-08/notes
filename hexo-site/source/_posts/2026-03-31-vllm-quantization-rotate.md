---
title: 量化中的 Rotate（旋转变换）技术及 vLLM 实现
date: 2026-03-31
tags: [vLLM, 量化]
---
...
```
## Q&A～
### 为什么旋转后区间长度没缩小，精度却提高了？

直觉上，旋转后的区间总长度（range）并没有缩小，甚至可能略有增大（不旋转时约 11.2，旋转后约 13.5）。但量化精度确实提高了，原因在于量化精度的瓶颈**不是区间长度，而是值的分布是否均匀地利用了量化级别**。

**不旋转时**：3 个正常值挤在 [0.8, 1.5] 这个很窄的范围内（仅占总区间的 6%），被迫共享 1~2 个量化级别，中间大量级别（2~6）完全浪费：

```
量化级别:  0    1    2    3    4    5    6    7
对应值:   0.0  1.71 3.43 5.14 6.86 8.57 10.3 12.0
          ↑↑↑                                  ↑
          三个值挤在这里                        离群值独占这里
          中间大片级别完全浪费！
```

**旋转后**：4 个值分散在不同的量化级别上，每个值都能被独立、准确地表示，没有级别被大片浪费。

关键在于两点共同作用：

1. **max(|x|) 降低**（从 12.0 降到 7.75，降低 35%）→ scale 变小（从 1.714 降到 1.107）→ 每个量化级别的间距更小 → 分辨率更高
2. **值的分布更均匀** → 量化级别被充分利用 → 没有任何维度被"牺牲"

### 为什么 scale 的计算要除以 7？

因为这里用的是**有符号 INT4 量化**。4 bit 能表示 2^4 = 16 个不同的值，有符号情况下（补码表示）范围是 **-8 ~ +7**，正数方向的最大值是 **7**。

scale 的计算方式：`scale = max(|x|) / 7`，目的是让最大的浮点值刚好映射到量化后的最大整数值 +7。

```
量化：   q = round(x / scale)     → 得到 -8 ~ +7 范围内的整数
反量化： x̂ = q × scale            → 恢复为浮点近似值
```

推广到其他位宽：
- 无符号 INT4（范围 0~15）→ 除以 15
- 有符号 INT8（范围 -128~+127）→ 除以 127
- 有符号 INTn → 除以 2^(n-1) - 1
