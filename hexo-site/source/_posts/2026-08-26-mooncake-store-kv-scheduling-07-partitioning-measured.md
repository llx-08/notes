...
延迟指出了方向。batch 4096 时 whole 是 5.67ms，拆 8 份是 13.91ms ≈ 8 × 1.74ms——子调用**几乎是串行的**，而不是并行。

我最初把这归因于「单 client 的 RPC 流水线深度」，并据此提出一条 TODO：把 store 调用做成真异步。
**后来把这条追到底，发现前一半的归因错了、后一半的结论也站不住。** 见下一节。

### 追到底：那个上限是 GIL 下的序列化，而且它不是部署的性质

三个假设逐个排除。

**假设 A：上限属于 client 对象（RPC 连接池深度）。** 若如此，同一进程内开 N 个 client 对象、
把调用分散过去就该突破。实测（同一进程、同一 master、每 client 4 线程）：

```
client 对象数      1          2          4          8
keys/s        1,747,301  1,600,375  1,833,710  1,440,443
```

**完全平的** → 上限不属于 client 对象，是**每进程**的。

**假设 B：GIL 全程持有。** 若如此，多线程等于单线程。实测（一进程、一 client、扫线程）：

```
线程数        1          2          4          8         16
keys/s     693,929  1,562,095  1,890,422  2,151,987  1,917,730
相对         1.00×      2.25×      2.72×      3.10×     2.76×(p50 2.19ms=排队)
```

多线程**有用**，最多 3.1 倍 → GIL 不是全程持有。

**假设 C（成立）：混合。** pybind 调用把 Python 参数转成 C++ 类型时**持有** GIL，
进入 RPC 等待时**释放**。按 Amdahl，比例 `f` 的时间持 GIL 则最大加速 `1/f`：

```
实测最大加速 3.1×  →  f ≈ 0.32
```

即**每次调用约 1/3 的时间花在 GIL 下把 256 个 Python 字符串转成 C++ vector<string>**，
这部分开多少线程都是串起来的。于是每进程在约 **2.1M keys/s** 见顶。数量级也自洽——
1.5-2M 个 Python 字符串/秒穿过 pybind 正是这类序列化的典型速率。这也顺带解释了 M1 里
「进程数近线性扩展」：**每个进程有自己的 GIL。**

### 所以「fan-out 付 50% 吞吐」是压测器伪影，不是部署代价

上面那个 2.1M 是**单进程压测器**的性质。真实部署里 keys/s 天然摊在与 TP rank 数一样多的
进程上，每个 rank 独立进程、独立 GIL：

```
                        聚合 keys/s   client 进程数   每进程        占每进程上限
实测集群（本地命中 90%）    5,758        6-14      约 400-1000      0.03%
百实例冷缓存外推         14,000,000      200        70,000         3.3%
```

即使那个「必然过载」的外推场景，每个客户端进程也只用到自己上限的 3.3%。而 M2 的 50%
是在 4 进程 × 4 线程、**已经压到 GIL 平台**的状态下测的：此时拆成 N 个子调用意味着 N 倍的
序列化工作挤在同一个 GIL 上。部署里离平台还有 30 倍，**这个吞吐代价不会出现。**

留下的是**延迟**代价：拆 N 份后要等最慢的返回，p50 涨约 2.4 倍，绝对值约 2ms，
对 2424ms 的请求是 0.08%。

**所以分区的正确定价是「约 2ms 查询延迟换 N× master 侧容量」，基本上是免费的**，
而不是我原先写的「付 50% 吞吐」。原来那条「别过度分区」的规则随之作废——
N 的选择应该由 master 侧容量需求决定，而不是被一个不存在的吞吐代价约束。

这是这条线上第二次「把工具的性质当成系统的性质」（第一次是用 `free` 读容器内存），
形状一样：**测量本身没错，错在它回答的不是我以为的那个问题。**
...
| 代价的性质 | **压测器伪影**：GIL 下序列化占 1/3（Amdahl f≈0.32），每进程 2.1M 见顶；部署每进程只用 3.3% | 客户端对象数扫描 + 线程扫描 |
| 部署里 fan-out 的真代价 | **只有约 2ms 查询延迟**（p50 ×2.4），吞吐代价不出现 | 上一行 |
...
原本这里写的下一步是「把 store 调用做成真异步」。**撤销，两个理由都是硬的：**

1. **做不到。** 这版 87 个方法全是同步的，没有任何 submit/poll/future 形式的接口。
   而包里那个看似现成的 `MooncakeDistributedStoreAsync`（`async_store.py`）实现是
   `loop.run_in_executor(None, func)`——和我们已经在用的 `asyncio.to_thread` 完全等价，
   并没有多任何东西。要真异步只能改 C++ 重编。
2. **不需要。** 它要修的是客户端吞吐，而部署里客户端只用到自己上限的 3.3%。
   瓶颈在 master 侧，不在客户端。

**正确的方向是减少发出去的 key 数，而不是加快发送**——也就是 C1（`kv_events` 负向过滤）。
