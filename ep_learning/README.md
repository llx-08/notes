...
> 已有专题笔记：[vllm_cross_node_expert_parallelism.md](../vllm_cross_node_expert_parallelism.md)、[rdma_learning_1.md](../rdma_learning_1.md)
...
| 7 | [07_practice_checklist.md](07_practice_checklist.md) | 动手实验、调试清单、常见坑 | 随练 |
...
> 「home / 原 rank」≠「唯一做 Attn 的 TP」：Attn 同 DP 内各 TP 都算同一批 token；详见 [01 §3.3](01_ep_fundamentals.md#33-token-原-rank--home指什么易混点)。
...
| All-to-All | MoE 下 token↔expert rank 的多对多交换；细讲见 [01a](01a_moe_all_to_all.md) |
| Logical / Physical Expert | 逻辑 expert（模型定义）vs 物理副本（EPLB 可复制热点） |
| DeepEP | DeepSeek 开源的 EP all-to-all 库（`deep_ep`），含 HT / LL 两套 kernel |
| DeepGEMM | DeepSeek FP8/FP4 GEMM 库；MoE 用 grouped contiguous；见 [02a](02a_deepgemm.md) |
...
- [vllm_cross_node_expert_parallelism.md](../vllm_cross_node_expert_parallelism.md)：跨机 EP 拓扑、后端表、OOM 陷阱——本系列的「实战专题」，学完 01–03 后精读。
